...
Uploads a dense or sparse matrix to a residency-capable backend and
returns the corresponding `aMatrix` object with a live resident binding.
This is most useful for repeated GPU work where paying the upload cost
once is preferable to relying on cold-path dispatch.
