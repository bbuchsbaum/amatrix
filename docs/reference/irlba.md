...
Wraps [`irlba::irlba()`](https://rdrr.io/pkg/irlba/man/irlba.html) with
an `adgeMatrix` input so that every Lanczos matrix-vector product routes
through the amatrix GPU dispatch path. The matrix `A` is kept resident
on device; consecutive matvecs in the Lanczos loop avoid host
round-trips.
...
- ...:
  ...
  `fastpath` is always forced to `FALSE` — the C fastpath bypasses S4
  dispatch and cannot be GPU-accelerated.
...
- implementation:
  ...
  [`irlba::irlba()`](https://rdrr.io/pkg/irlba/man/irlba.html) wrapper
  behavior. `"block"` routes to
...
Same structure as
[`irlba::irlba()`](https://rdrr.io/pkg/irlba/man/irlba.html): a list
with components `d`, `u`, `v`, `iter`, `mprod`.
