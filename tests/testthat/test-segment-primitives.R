...
#
# Tests for segment_sum() and segment_mean() — amatrix-ylo
#
...
test_that("segment_sum: unsorted labels", {
  set.seed(2)
...
test_that("segment_sum: large K (K = n/2)", {
  set.seed(6)
  n      <- 40L; K <- 20L
...
# ── segment_mean — correctness ──────────────────────────────────────────

test_that("segment_mean: standard random labels, plain matrix input", {
  set.seed(10)
...
test_that("segment_mean: unsorted labels", {
  set.seed(11)
...
test_that("segment_mean: empty cluster → NA row", {
  set.seed(12)
  X      <- matrix(rnorm(60 * 5), 60, 5)
  labels <- rep(c(1L, 3L), 30L)           # label 2 absent
  ref    <- .ref_segment_mean(X, labels, 3L)
  got    <- segment_mean(X, labels, 3L)
  if (inherits(got, "adgeMatrix")) got <- as.matrix(got)
...
test_that("segment_mean: K=1", {
  set.seed(13)
  X      <- matrix(rnorm(80 * 4), 80, 4)
  labels <- rep(1L, 80L)
  ref    <- matrix(colMeans(X), 1L, 4L)
  got    <- segment_mean(X, labels, 1L)
  if (inherits(got, "adgeMatrix")) got <- as.matrix(got)
  expect_equal(got, ref, tolerance = .seg_cpu_tol)
})

test_that("segment_mean: all-same labels (every row in group 2)", {
  set.seed(14)
...
test_that("segment_mean: wide matrix (p > n)", {
  set.seed(15)
...
test_that("segment_mean: large K", {
  set.seed(16)
  n      <- 100L; K <- 50L
...
test_that("segment_sum: adgeMatrix input with cpu backend falls back correctly", {
  set.seed(20)
  ...
  got    <- segment_sum(X, labels, 6L)
  if (inherits(got, "adgeMatrix")) got <- as.matrix(got)
  expect_equal(got, ref, tolerance = .seg_cpu_tol)
})

test_that("segment_mean: adgeMatrix input with cpu backend falls back correctly", {
  set.seed(21)
...
for (.bk in .seg_backends) {
  local({
    ...
    test_that(sprintf("segment_sum: %s backend returns resident adgeMatrix", bk), {
      set.seed(30)
      n <- 500L; p <- 20L; K <- 10L
      ...
      expect_true(inherits(result, "adgeMatrix"),
                  info = sprintf("%s segment_sum must return adgeMatrix", bk))
      expect_equal(dim(result), c(K, p))
      ref <- .ref_segment_sum(X_mat, labels, K)
      expect_equal(as.matrix(result), ref, tolerance = tol,
                   label = sprintf("%s segment_sum values", bk))
    })

    test_that(sprintf("segment_mean: %s backend returns resident adgeMatrix", bk), {
      set.seed(31)
      n <- 500L; p <- 20L; K <- 10L
      X_mat  <- matrix(rnorm(n * p), n, p)
      X      <- adgeMatrix(X_mat, preferred_backend = bk, precision = "fast")
      labels <- sample.int(K, n, replace = TRUE)
      result <- segment_mean(X, labels, K)
      expect_true(inherits(result, "adgeMatrix"),
                  info = sprintf("%s segment_mean must return adgeMatrix", bk))
      expect_equal(dim(result), c(K, p))
      ref <- .ref_segment_mean(X_mat, labels, K)
      got <- as.matrix(result)
      expect_equal(is.na(got), is.na(ref))
      expect_equal(got[!is.na(ref)], ref[!is.na(ref)], tolerance = tol,
                   label = sprintf("%s segment_mean values", bk))
    })

    test_that(sprintf("segment_mean: %s empty cluster → NaN/NA propagation", bk), {
      set.seed(32)
      n <- 200L; p <- 8L; K <- 5L
      X_mat  <- matrix(rnorm(n * p), n, p)
      X      <- adgeMatrix(X_mat, preferred_backend = bk, precision = "fast")
    ...
    test_that(sprintf("segment_mean: %s non-contiguous resident input", bk), {
      set.seed(33)
      n <- 300L; p <- 15L; K <- 8L
      X_mat  <- matrix(rnorm(n * p), n, p)
      X      <- adgeMatrix(X_mat, preferred_backend = bk, precision = "fast")
      ...
      labels <- sample.int(K, n, replace = TRUE)
      X2_mat <- as.matrix(X2)
      result <- segment_mean(X2, labels, K)
