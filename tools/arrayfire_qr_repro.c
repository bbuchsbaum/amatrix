...
int main(int argc, char** argv) {
  ...
  float* q_host = NULL;
  float* r_host = NULL;
  af_array ax = 0, ax_t = 0, q = 0, r = 0, tau = 0;
  af_array q_t = 0, r_t = 0;
  ...
  if (backend != AF_BACKEND_DEFAULT) {
    ...
    if (err != AF_SUCCESS) {
      fail_af("af_set_backend", err);
    }
  }

  err = af_get_active_backend(&active);
  if (err != AF_SUCCESS) {
    fail_af("af_get_active_backend", err);
  }

  err = af_is_lapack_available(&lapack);
  if (err != AF_SUCCESS) {
    fail_af("af_is_lapack_available", err);
  }
  ...
  if (err != AF_SUCCESS) {
    fail_af("af_transpose(q)", err);
  }

  printf("[af_qr_repro] transposed q\n");
  fflush(stdout);

  err = af_transpose(&r_t, r, false);
  if (err != AF_SUCCESS) {
    fail_af("af_transpose(r)", err);
  }

  printf("[af_qr_repro] transposed r\n");
  fflush(stdout);
  ...
  if (err != AF_SUCCESS) {
    fail_af("af_get_data_ptr(q)", err);
  }

  printf("[af_qr_repro] materialized q\n");
  fflush(stdout);

  err = af_get_data_ptr(r_host, r_t);
  if (err != AF_SUCCESS) {
    fail_af("af_get_data_ptr(r)", err);
  }

  printf("[af_qr_repro] materialized r\n");
  fflush(stdout);
